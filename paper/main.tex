...
matches \tc{} exactly (geometric mean 1.00$\times$) and all four paths reach 87 to 88\%
of peak DRAM bandwidth, showing that the method reports no gain where memory bandwidth
...
Every path converges to the same ceiling: all twenty-one measurements fall in
392 to 398~$\mu$s, i.e.\ 86.8 to 88.2\% of peak, indistinguishable from the 88.6\% of a pure
device copy. For an operator costing one read and one write this is the hardware limit.
The two operators with genuine headroom, eager softmax at 75.4\% and layernorm at
58.4\%, which materialize intermediates for their reductions, are closed identically by
